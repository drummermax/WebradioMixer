		...
		public void send(MidiMessage msg, long timeStamp) {
			...
			if (midikey.isFader()) {
				if (MidiController.getInstance().getCurrentTimestamp()
					...
					System.out.println("time return with " + (MidiController.getInstance().getCurrentTimestamp()
							- MidiController.getInstance().getTimestamp()[note_cc - 77]));
					return;
				} else {
		...
		public void run() {
			...
			while (true) {
				if (Mairlist.getInstance().getMairlistPlayerStatePlayer1() == MairlistPlayerState.EMPTY) {
					...
					MIDIKey.BUTTONDOWN3.setLEDColor(MIDIKey.LEDColor.YELLOW); // grün
				...
				if (MixingDesk.getInstance().isCartwallActive()) {
					MIDIKey.BUTTONDOWN5.setLEDColor(MIDIKey.LEDColor.GREEN); // grün
				} else {
