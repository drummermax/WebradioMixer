...
public class Output {
	...
	boolean activateLatencyCompensation;
	FIFO latencyCompensationFIFO;
	...
	public Output(AudioFormat audioFormat, int mixerInfosIndex, boolean activateLatencyCompensation) {
		...
		this.activateLatencyCompensation = activateLatencyCompensation;
		...
		int latencyCompensationBufferSize;

		if (activateLatencyCompensation) {
			latencyCompensationBufferSize = ((int) (sampleRate * GlobalVariables.getInstance().getLatencyCompensation()
					/ (1000 * GlobalVariables.getInstance().getBufferSize()))) + 1;
		} else {
			latencyCompensationBufferSize = 1;
		}

		System.out.println("latency compensation buffer size: " + latencyCompensationBufferSize);

		latencyCompensationFIFO = new FIFO(latencyCompensationBufferSize);
	...
	public void setInputData(byte[] inputData) {
		...
		latencyCompensationFIFO.putElement(inputData.clone());

		sourceDataLineOut.write((byte[]) latencyCompensationFIFO.getBuffer()[0], 0, inputDataLength);																						// 1
	}
